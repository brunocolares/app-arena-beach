...
async function buscarHorariosDisponiveis(quadraId: string, data: string): Promise<SlotHorario[]> {
  const r = await api.get(`/horarios-disponiveis?quadra_id=${quadraId}&data=${data}`);
  return r.data;
}
...
function calcularBlocoHorario(horarios: string[]): { inicio: string; fim: string } {
  const ordenados = [...horarios].sort((a, b) => horaParaMinutos(a) - horaParaMinutos(b));
...
export function TelaHorarios() {
  ...
  function aoTocarSlot(horario: string) {
    ...
    if (horariosSelecionados.includes(horario)) {
      ...
      if (ePrimeiro || eUltimo) {
        ...
        Alert.alert('Atenção', 'Só é possível remover o primeiro ou o último horário do bloco.');
      }
      return;
    }
    ...
    const adjacenteNoFim = saoConsecutivos(ultimo, horario) && horaParaMinutos(horario) > horaParaMinutos(ultimo);
    const adjacenteNoInicio = saoConsecutivos(horario, primeiro) && horaParaMinutos(horario) < horaParaMinutos(primeiro);
  ...
  const blocoHorario = horariosSelecionados.length > 0
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={estilos.scroll}>
        ...
        <View style={estilos.secao}>
          ...
          <Text style={estilos.dicaMulti}>
            💡 Toque em horários consecutivos para reservar mais de uma hora seguida.
          </Text>

          {isLoading ? (
            <View style={estilos.centralizado}>
              <ActivityIndicator color={Cores.primaria} />
              <Text style={estilos.textoCarregando}>Verificando disponibilidade...</Text>
            </View>
          ) : (
            <View style={estilos.gridSlots}>
              {(slots || []).map((slot) => {
                ...
                const ehAdjacente =
                  ...
                  slot.disponivel &&
                  horariosSelecionados.length > 0 &&
                  (saoConsecutivos(ultimoSel, slot.horario) && horaParaMinutos(slot.horario) > horaParaMinutos(ultimoSel) ||
                   saoConsecutivos(slot.horario, primeiroSel) && horaParaMinutos(slot.horario) < horaParaMinutos(primeiroSel));
                ...
                return (
                  ...
                  >
                    ...
                    {!slot.disponivel && <Text style={estilos.textoOcupado}>Ocupado</Text>}
                    {ehAdjacente && <Text style={estilos.textoAdjacente}>+ add</Text>}
                  </TouchableOpacity>
                );
              })}
              {(!slots || slots.length === 0) && (
                <Text style={estilos.textoVazio}>Nenhum horário disponível neste dia.</Text>
              )}
            </View>
          )}
        </View>
        ...
        {horariosSelecionados.length > 0 && blocoHorario && (
          <View style={estilos.resumo}>
            ...
            <ItemResumo label="Data" valor={formatarData(diaSelecionado.data)} />
            <ItemResumo
              label="Período"
...
function ItemResumo({ label, valor, destaque }: { label: string; valor: string; destaque?: boolean }) {
  return (
    <View style={estilosItem.linha}>
      <Text style={[estilosItem.label, destaque && estilosItem.labelDestaque]}>{label}</Text>
...
const estilosItem = StyleSheet.create({
  ...
  labelDestaque: { color: Cores.textoSecundario, fontSize: 12, fontStyle: 'italic' },
...
const estilos = StyleSheet.create({
  ...
  centralizado: { alignItems: 'center', paddingVertical: Espacamento.xl, gap: 8 },
  ...
  separadorResumo: { height: 1, backgroundColor: Cores.borda, marginVertical: 6 },
  linhaTotalResumo: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
