...
async function salvarQuadra(dados: Partial<Quadra> & { id?: string }): Promise<Quadra> {
  if (dados.id) {
...
export function TelaFormularioQuadra() {
  ...
  const [preco, setPreco] = useState(quadraExistente ? String(quadraExistente.preco_hora) : '');
  const [esporteId, setEsporteId] = useState(quadraExistente?.esporte_id || '');
  const [fotos, setFotos] = useState<string[]>(quadraExistente?.fotos || []);
  const [diferenciais, setDiferenciais] = useState<string[]>(quadraExistente?.diferenciais || []);
  const [novoDiferencial, setNovoDiferencial] = useState('');
  const [erros, setErros] = useState<Record<string, string>>({});

  const { data: esportes } = useQuery({ queryKey: ['esportes'], queryFn: buscarEsportes });
  ...
  const mutacao = useMutation({
    ...
    onSuccess: () => {
      ...
      Alert.alert('Sucesso', quadraExistente ? 'Quadra atualizada!' : 'Quadra criada!');
      navegacao.goBack();
    },
  ...
  async function aoSelecionarFotos() {
    ...
    if (!permissao.granted) {
      Alert.alert('Permissão negada', 'Permita o acesso à galeria para adicionar fotos.');
      return;
    }
  ...
  function validar(): boolean {
    ...
    if (!esporteId) e.esporte = 'Selecione um esporte';
    setErros(e);
  ...
  return (
    <View style={estilos.container}>
      ...
      <ScrollView contentContainerStyle={estilos.scroll} showsVerticalScrollIndicator={false}>
        ...
        {/* Seletor de Esporte */}
        <Text style={estilos.labelInput}>Esporte *</Text>
        <View style={estilos.containerEsportes}>
          {(esportes || []).map(e => (
            <TouchableOpacity
              key={e.id}
              style={[estilos.cardEsporte, esporteId === e.id && estilos.cardEsporteAtivo]}
              onPress={() => setEsporteId(e.id)}
            >
              <Text style={[estilos.textoEsporte, esporteId === e.id && { color: Cores.texto }]}>
                {e.nome_esporte}
              </Text>
            </TouchableOpacity>
          ))}
        </View>
        {erros.esporte ? <Text style={estilos.textoErro}>{erros.esporte}</Text> : null}
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            {fotos.map((uri, idx) => (
              <View key={idx} style={estilos.containerFoto}>
                ...
                <TouchableOpacity style={estilos.botaoRemoverFoto} onPress={() => aoRemoverFoto(uri)}>
                  <Text style={estilos.textoRemoverFoto}>✕</Text>
                </TouchableOpacity>
              </View>
            ))}
            <TouchableOpacity style={estilos.botaoAdicionarFoto} onPress={aoSelecionarFotos}>
        ...
        <View style={estilos.secao}>
          ...
          <View style={estilos.adicionarDiferencial}>
            ...
            />
            <TouchableOpacity style={estilos.botaoAddTag} onPress={aoAdicionarDiferencial}>
              <Text style={estilos.textoAddTag}>+</Text>
            </TouchableOpacity>
          </View>
          <View style={estilos.tagContainer}>
            {diferenciais.map((d, i) => (
              <TouchableOpacity key={i} style={estilos.tag} onPress={() => setDiferenciais(prev => prev.filter((_, idx) => idx !== i))}>
        ...
        <View style={estilos.rodape}>
          <Botao titulo={quadraExistente ? 'Salvar alterações' : 'Cadastrar quadra'} onPress={aoSalvar} carregando={mutacao.isPending} />
        </View>
      </ScrollView>
    </View>
  );
...
const estilos = StyleSheet.create({
  ...
  labelInput: { color: Cores.textoSecundario, fontSize: 14, fontWeight: '500', marginBottom: 8 },
  containerEsportes: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: Espacamento.md },
  cardEsporte: {
  ...
  cardEsporteAtivo: { backgroundColor: Cores.primaria, borderColor: Cores.primaria },
  textoEsporte: { color: Cores.textoSecundario, fontSize: 14 },
  textoErro: { color: Cores.erro, fontSize: 12, marginTop: -8, marginBottom: Espacamento.md },
  ...
  adicionarDiferencial: { flexDirection: 'row', gap: 8, alignItems: 'flex-end' },
  botaoAddTag: {
    width: 44,
    height: 44,
