...
import {
  ErroConflito, ErroNaoAutorizado, ErroNaoEncontrado, ErroValidacao
...
export class ReservaController {
  ...
  async horariosDisponiveis(req: Request, res: Response, next: NextFunction) {
    try {
      const { quadra_id, data } = req.query as { quadra_id: string; data: string };
      if (!quadra_id || !data) {
        return res.status(400).json({ mensagem: 'quadra_id e data são obrigatórios.' });
      }
  ...
  async criar(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (dataPassada(data_reserva, hora_inicio)) {
        throw new ErroValidacao('Não é possível agendar horários que já passaram.');
      }
  ...
  async cancelar(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!isAdmin && reserva.usuario_id !== usuarioId) {
        throw new ErroNaoAutorizado('Você não tem permissão para cancelar esta reserva.');
      }
  ...
  async criarBloqueio(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (dataPassada(data, hora_inicio)) {
        throw new ErroValidacao('Não é possível bloquear horários que já passaram.');
      }
      ...
      if (conflito) throw new ErroConflito('Já existe uma reserva neste horário.');
  ...
  async agendaDiaria(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      return res.json({ quadras: quadras.map(q => ({
        id: q.id,
