...
async function popularBancoDeDados() {
  ...
  // Admin padrão
  const adminExistente = await repoUsuario.findOne({ where: { email: 'admin@arena.com' } });
  if (!adminExistente) {
  ...
  // Usuário de teste
  const userExistente = await repoUsuario.findOne({ where: { email: 'joao@email.com' } });
  if (!userExistente) {
  ...
  if (esportesExistentes === 0) {
    const esportes = [
      ...
      { nome_esporte: 'Futebol Society', imagem_url: '', preco_partir: 'R$ 120/h' },
      { nome_esporte: 'Vôlei de Praia', imagem_url: '', preco_partir: 'R$ 70/h' },
    ...
    console.log(`✅ ${esportesExistentes === 0 ? esportes.length : 0} esportes criados`);
